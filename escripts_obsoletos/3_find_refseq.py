...
for entrada in entrada_marcas_cooloc.split('\n'):
    if len(entrada)>0:
        gene_name=entrada.split('\t')[5]
        gene_chr=entrada.split('\t')[6]
        gene_start=entrada.split('\t')[7]
        gene_end=entrada.split('\t')[8]
            ...
            if len(refs)>0:
                ...
                if chr_query == chr_ref:
                    if int(procurar_query)> int(process_ref[4]) and int(procurar_query)< int(process_ref[5]):
                        ...
                        saida_formatada= '\t'.join(((procurar_query),(process_ref[4]),(process_ref[5]),process_ref[1],process_ref[3],gene_name,gene_chr,gene_start,gene_end))
