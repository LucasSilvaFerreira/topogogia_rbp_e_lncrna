...
class teste(thread_processador):
    ...
    def funcao(self,inicio,fim):
        ...
        self.chia_pet_file=open('wgEncodeGisChiaPetHct116Pol2InteractionsRep1.bed','r').read()

        for linha in self.arquivo_array[int(inicio):int(fim)]:
            if len(linha)>0:
                if 'ENST' in linha:
                    #print linha
                    cromossomo= linha.split()[4]
                    start=linha.split()[6]
                    end=linha.split()[7]
                    name=linha.split()[0]
                    #print '->',name
...
teste('GSM880698_Enriched_LncRNAs.txt',4,'saida_procurar_colocalizacao_lncrna.txt')
